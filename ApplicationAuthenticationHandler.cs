...
namespace FavoriteDentists
{
    public class ApplicationAuthenticationHandler : DelegatingHandler
    {
        ...
        protected override System.Threading.Tasks.Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, System.Threading.CancellationToken cancellationToken)
        {
            ...
            IEnumerable<string> fdApiKeyHeaderValues = null;

            // Checking the Header values
            if (request.Headers.TryGetValues("X-fdAppApiKey", out fdApiKeyHeaderValues))
            {
                string[] apiKeyHeaderValue = fdApiKeyHeaderValues.First().Split(':');
